...
soil_types = {"measured","loam","clay_loam"};
...
for p = 1:length(top_percent_pixels)
    ...
    if p == 1
        npixels = round(top_percent_pixels(p)*npervious/100);
    else
        npixels = round(top_percent_pixels(p)*npervious/100) - ...
            round(top_percent_pixels(p-1)*npervious/100);
    end
    all_ind = find(parcelCover==0);
    amend_ind = all_ind(randsample(length(parcelCover(parcelCover==0)),npixels));
    parcelCover(amend_ind) = 10;
    ...
    for s = 1:length(soil_types)
        soil = soil_types{s};
        runname = sprintf('amend_pixels_%s_%s',soil,num2str(top_percent_pixels(p)));
        ...
        load(sprintf('%s/soil_base_%s',soil_dir, soil))
